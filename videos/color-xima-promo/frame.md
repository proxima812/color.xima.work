...
colors:
  # Ground = the product's own canvas (#09090B), not its text colour.
  ink-black: "#09090B"
  ink-black-alt: "#111114"
  # The "accent register" is a near-white plate, deliberately achromatic:
  # in this video the ONLY chroma on screen is a real catalogue gradient.
  fire-orange: "#FAFAFA"
  cream: "#F4F4F5"
  cream-muted: "#9F9FA9"
  cream-hint: "#71717B"
  border-dark: "#27272A"
  ink-on-orange-muted: "rgba(9,9,11,0.72)"
  ink-on-orange-hint: "rgba(9,9,11,0.55)"
  ink-on-orange-faint: "rgba(9,9,11,0.40)"
  ink-on-orange-border: "rgba(9,9,11,0.18)"
...
typography:
  ...
  # — display / hero ramp (-apple-system, lowercase, negative tracking) —
  h3:      { fontFamily: "Inter", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Inter", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.03em", lower: true }
  h2:      { fontFamily: "Inter", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.03em", lower: true }
  stat-value:{ fontFamily: "Inter", cqw: 5.5, weight: 700, lineHeight: 1.0, tracking: "-0.03em" }
  h1:      { fontFamily: "Inter", cqw: 7.5, weight: 700, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Inter", cqw: 7.5, weight: 700, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Inter", cqw: 10.0, weight: 700, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Inter", cqw: 10.5, weight: 700, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  display: { fontFamily: "Inter", cqw: 13.0, weight: 700, lineHeight: 0.88, tracking: "-0.03em", lower: true }
...
- **Fonts** — **Inter Bold (700) at -3% tracking** (display + body) and **IBM Plex Mono** (chrome/labels), both webfonts loaded per frame.
  The capture reported the brand face as `-apple-system`, which is a system stack, not a webfont: it cannot be
  embedded, renders differently per machine, and ships no weight above 700. this video is silent, so typography carries the entire
  piece and needs a real embeddable webfont. Inter Bold at -3% tracking was specified directly by the user and
  overrides both the automatic remix and the preset's own Barlow 900 display ramp.
- **Weights** — display and body both sit on Inter 700; the mono chrome stays IBM Plex Mono 500.
- **Colors** — use the frontmatter hex. The automatic remix inverted the system (it read the brand's *text*
  colour `#F4F4F5` as the ground and produced a light frame for a near-black product); that inversion is
  reverted. Ground is the site's real canvas `#09090B`.
- **The accent is achromatic on purpose.** The product's UI is monochrome and all of its colour lives in the
  gradients themselves. So the `fire-orange` slot is a near-white plate, and the only chroma allowed on screen
  is a real gradient from `assets/gradients.css`. Never introduce a decorative brand hue — it would compete
  with the one thing the video is about.
...
**-apple-system** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase -apple-system 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (-apple-system body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (-apple-system `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **-apple-system display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
-apple-system `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
-apple-system lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word -apple-system `display`/`h1`
...
row of three top-border-only stat-cards — big -apple-system-900 numeral in `{colors.fire-orange}`, -apple-system
...
**Ground** ink-black. **Composes** fadelist (3 stacked -apple-system-900 words at 1.0/0.5/0.22), fadelist-title.
...
-apple-system `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every -apple-system display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase -apple-system display; never add a second accent color.
...
- No serif companion; chrome is never -apple-system.
...
- **Type** — -apple-system lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **-apple-system + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
